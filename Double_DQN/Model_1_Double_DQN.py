...
def evaluate(agent, env, initial_balance=10000):
    '''
    ...
    while not done:
        action = agent.act(state) #  (comprar/vender/mantener) 
        next_state, reward, done, info = env.step(action)
        
        # Desnormalizar precio de 0-1 a USD
...
while not done:
    ...
    if action == 2 and portfolio > 0:
        ...
        positions += (buy_amount * (1 - test_env.commission)) / current_price
        portfolio -= buy_amount
    elif action == 0 and positions > 0:
        sell_amount = positions * 0.1
        portfolio += (sell_amount * current_price) * (1 - test_env.commission)
        positions -= sell_amount
