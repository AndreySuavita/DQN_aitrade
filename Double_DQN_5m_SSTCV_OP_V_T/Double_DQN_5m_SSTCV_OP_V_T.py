...
def wait_until_next_hour_with_offset(offset_seconds=15):
    ...
    target_time = next_5min + timedelta(seconds=offset_seconds)
...
def evaluate(agent, env, scaler, initial_balance=10000, binance_on=False):
    ...
    while not done:     
        ...
        if action == 2 and portfolio > 0: #buy
            buy_amount = portfolio * 0.03 # invest 10% of the portfolio
            ...
            sell_amount = positions * 0.03 # Sell 10% of the ETH held
...
def metrics(portfolio_history, test_return, price_history, actions_history, initial_balance, mean_fold_results, std_fold_results):
    ...
    sharpe_ratio = np.mean(returns) / np.std(returns) * np.sqrt(252*24*12) # 12 velas/hora
...
if __name__ == "__main__":
    ...
    data_array, df, scaler = load_and_preprocess_data(window_size=window_size, filepath='C:\\Andrey\\Kakua_Projets\\Trading\\Bot_RL_v1\\Datasets\\historical_01-12-2024_to_01-05-2025_5m_ETHUSDT.csv')
